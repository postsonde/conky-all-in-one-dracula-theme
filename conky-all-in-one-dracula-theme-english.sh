#!/bin/bash
# =====================================================================
# ALL-IN-ONE CONKY DRACULA-THEME INSTALLATION SCRIPT
# =====================================================================

# 1. DETECT USERNAME DYNAMICALLY
CURRENT_USER=$(whoami)
...
# 3. CREATE TO-DO TEXT FILE (Only if it doesn't exist)
if [ ! -f ~/Dokumente/.conkytodo.txt ]; then
cat << 'EOF' > ~/Dokumente/.conkytodo.txt
[ ] Test Bluetooth connection

[ ] Try battery warning color

[ ] Enjoy a hot coffee
EOF
echo "Hidden To-Do file created at ~/Dokumente/.conkytodo.txt"
fi

# 4. WRITE LEFT CONFIG (MY TASKS / TO-DO LIST)
...
conky.text = [[
${color1}${font DejaVu Sans:bold:size=12}MY TASKS${font}${color}
${color1}${hr 1}${color}
${execp cat ~/Dokumente/.conkytodo.txt | sed 's/^/  /'}
]];
EOF

# 5. WRITE RIGHT CONFIG (SYSTEM MONITOR - 310PX)
...
${color1}${font DejaVu Sans:bold:size=8}STATUS & METRICS${font}${color}
${color}Bluetooth Status: ${alignr}${color2}${execi 5 bluetoothctl show | grep -q "Powered: yes" && (bluetoothctl info | grep "Name:" | cut -d' ' -f2- || echo "Enabled") || echo "Disabled" | cut -c1-18}
${color}Volume Level: ${alignr}${color2}${execi 1 pactl get-sink-volume @DEFAULT_SINK@ | grep -oP '\d+%' | head -1}
${color}Ping (Google): ${alignr}${color2}${execi 60 ping -c 1 8.8.8.8 2>/dev/null | grep -oP 'time=\K[\d.]+' | head -1} ms
${color}Notifications: ${alignr}${color2}${execi 5 qdbus org.kde.NotificationArea /NotificationArea org.kde.NotificationArea.activeNotifications 2>/dev/null | wc -l}
${color}Updates (Apt): ${alignr}${color2}${execi 3600 apt list --upgradable 2>/dev/null | grep -c upgradable} pending
${color1}${hr 1}${color}

${color1}${font DejaVu Sans:bold:size=10}PROCESSOR (CPU)${font}${color}
${color2}${execi 600 lscpu | grep 'Modellname\|Model name' | cut -d':' -f2 | sed -e 's/^[ \t]*//' | cut -c1-32}${color}
${color}CPU Load:${color2} ${cpu cpu0}% ${alignr}${color}Temp: ${if_match ${acpitemp} >= 65}${color3}${else}${color2}${endif}${acpitemp}°C${color}
${color2}${cpubar cpu0 4}${color}

${color}CPU Cores:${color2}" >> ~/.conky/clean-all-in-one.conf
...
${color}Top Processes (CPU 1-5):${goto 165}Top Processes (CPU 6-10):${color5}
...
${color1}${font DejaVu Sans:bold:size=10}MEMORY (RAM)${font}${color}
...
${color}Clipboard Size: ${alignr}${color2}${execi 2 xclip -selection clipboard -o 2>/dev/null | wc -c | awk '{print $1" Bytes"}'}

${color}Top Processes (RAM 1-5):${goto 165}Top Processes (RAM 6-10):${color5}
...
${color1}${font DejaVu Sans:bold:size=10}MEDIA & STORAGE${font}${color}
...
${execp mount | grep -E '/media/|/run/media/' | awk '{print "${color}Drive: ${color2}" $3 "\n${color2}${fs_bar 4 " $3 "}${color}"}' 2>/dev/null}

${color1}${font DejaVu Sans:bold:size=10}POWER MANAGEMENT (BATTERY)${font}${color}
${if_existing /sys/class/power_supply/BAT0}${if_match ${battery_percent BAT0} < 20}${color3}${else}${color2}${endif}Battery 1: ${battery_percent BAT0}% ${alignr}${battery_bar 4,100 BAT0}${endif}${color}
${if_existing /sys/class/power_supply/BAT1}${if_match ${battery_percent BAT1} < 20}${color3}${else}${color2}${endif}Battery 2: ${battery_percent BAT1}% ${alignr}${battery_bar 4,100 BAT1}${endif}

${color1}${font DejaVu Sans:bold:size=10}NETWORK & INFRASTRUCTURE${font}${color}
${if_existing /proc/net/route wlan0}${color}Local IP (WiFi): ${alignr}${color2}${addr wlan0}${endif}${if_existing /proc/net/route wlp2s0}${color}Local IP (WiFi): ${alignr}${color2}${addr wlp2s0}${endif}${if_existing /proc/net/route eth0}${color}Local IP (Lan): ${alignr}${color2}${addr eth0}${endif}${if_existing /proc/net/route enp3s0}${color}Local IP (Lan): ${alignr}${color2}${addr enp3s0}${endif}
${color}External IP: ${alignr}${color2}${execi 3600 curl -s -4 icanhazip.com || echo "Error"}
...
# 7. CREATE KDE MENU APPLICATION LAUNCHER
cat << EOF > ~/.local/share/applications/conkytodo.desktop
[Desktop Entry]
Name=Conky ToDo List
Comment=Opens the hidden task list
...
# 10. RESTART CONKY TO APPLY CHANGES
killall conky 2>/dev/null && sleep 2 && nohup ~/.conky/conky-startup.sh >/dev/null 2>&1 & disown

echo "====================================================================="
echo "=== DRACULA ALL-IN-ONE SYSTEM SUCCESSFULLY INSTALLED! ==="
echo "====================================================================="
